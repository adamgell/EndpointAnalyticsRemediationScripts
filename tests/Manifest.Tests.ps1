BeforeAll {
    ...
    function Get-RepositoryRelativePath {
        ...
        return $Path.Substring($scriptRoot.Length + 1).Replace('\', '/')
    }
...
Describe 'Catalog script manifests' {
    ...
    It 'uses exact mapped scenarios, roles, sources, and counterparts' {
        ...
        foreach ($row in $pathMap.Paths) {
            $manifestPath = Join-Path $scriptRoot ([IO.Path]::ChangeExtension($row.NewPath, '.psd1'))
    ...
    It 'uses symmetric pairs and exactly seven detection-only scenarios' {
        ...
        foreach ($script in $scripts) {
            ...
            $paired++
            $counterpartPath = Join-Path $scriptRoot $manifest.Identity.Counterpart
    ...
    It 'uses immutable unique IDs and stable reviewed versions' {
        ...
        (Import-PowerShellDataFile "$scriptRoot/Activate-Numlock/Detect-Activate-Numlock.psd1").Id |
            Should -BeExactly 'b82ba3eb-f5f1-5aa1-87bc-8934240b8cdd'
        (Import-PowerShellDataFile "$scriptRoot/Remove-New-Outlook/Detect-Remove-New-Outlook.psd1").Id |
            Should -BeExactly '433c1557-c869-5a1f-9721-4625700f2976'
        (Import-PowerShellDataFile "$scriptRoot/Remove-Silverlight/Detect-Remove-Silverlight.psd1").Id |
            Should -BeExactly '4317dd12-54dc-5265-9bcf-c91e5af8757d'
        (Import-PowerShellDataFile "$scriptRoot/Winget-Update-All/Remediate-Winget-Update-All.psd1").Id |
...
Describe 'Deterministic manifest generation' {
    ...
    It 'regenerates all 271 sidecars byte-identically' {
        ...
        foreach ($relativePath in $firstRelativePaths) {
            ...
            $catalogHash = (Get-FileHash -Algorithm SHA256 -LiteralPath (Join-Path $scriptRoot $relativePath)).Hash
