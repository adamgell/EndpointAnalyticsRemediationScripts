...
function Repair-GetRealTimeProtection {
    [CmdletBinding()]
    param(
        [scriptblock] $GetStatus = { Get-MpComputerStatus -ErrorAction Stop },
    ...
    $before = Test-GetRealTimeProtection -GetStatus $GetStatus
    ...
    try {
        $null = & $SetPreference ([ordered]@{ DisableRealtimeMonitoring = $false })
    }
    catch {
        return [pscustomobject]@{
            Succeeded = $false
    ...
    if ($null -ne $after.Error -or -not $after.Compliant) {
        $error = if ($null -ne $after.Error) {
            ...
            [pscustomobject]@{
                ...
                Message = 'Real-time protection remained disabled after remediation.'
            }
        }
        return [pscustomobject]@{
            Succeeded = $false
...
if ($MyInvocation.InvocationName -ne '.') {
    $result = Repair-GetRealTimeProtection
