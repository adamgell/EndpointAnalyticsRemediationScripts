...
function ConvertTo-LSAProtectionRepairState {
    param(
        $Record,
        [string]$Name
    )
...
function Get-LSAProtectionRepairObservation {
    ...
    try {
        ...
        $normalized = ConvertTo-LSAProtectionRepairState -Record $record -Name $name
...
function Repair-GetLSAProtection {
    [CmdletBinding()]
    param(
    ...
    $value = 1
    $type = 'DWORD'
    $before = Get-LSAProtectionRepairObservation -GetRegistry $GetRegistry
    $state = [pscustomobject][ordered]@{
        Path = $path
        Name = $name
        ExpectedValue = $value
    ...
    $after = Get-LSAProtectionRepairObservation -GetRegistry $GetRegistry
...
if ($MyInvocation.InvocationName -ne '.') {
    $result = Repair-GetLSAProtection
