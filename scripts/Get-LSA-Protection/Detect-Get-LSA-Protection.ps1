...
function ConvertTo-LSAProtectionState {
    param(
...
function Test-GetLSAProtection {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $false)]
        [Alias('GetState')]
    ...
    try {
        ...
        $normalized = ConvertTo-LSAProtectionState -Record $observed -Name $name
