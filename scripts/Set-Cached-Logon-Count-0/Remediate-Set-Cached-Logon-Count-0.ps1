...
Description: Windows NT may use a cache to store the last interactive logon (i.e. console logon),
to provide a safe logon for the host in the event that the Domain Controller goes down. This feature
is currently activated on this host.
...
function ConvertTo-CachedLogonCountRepairState {
    param(
        $Record,
        [string]$Name
    )
...
function Get-CachedLogonCountRepairObservation {
    ...
    try {
        ...
        $normalized = ConvertTo-CachedLogonCountRepairState -Record $record -Name $name
...
function Repair-SetCachedLogonCount0 {
    [CmdletBinding()]
    param(
    ...
    $value = 0
    $type = 'REG_SZ'
    $before = Get-CachedLogonCountRepairObservation -GetRegistry $GetRegistry
    $state = [pscustomobject][ordered]@{
        Path = $path
        Name = $name
        ExpectedValue = $value
    ...
    $after = Get-CachedLogonCountRepairObservation -GetRegistry $GetRegistry
