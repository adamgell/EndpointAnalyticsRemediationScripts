...
function ConvertTo-CachedLogonCountState {
    param(
...
function Test-SetCachedLogonCount0 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $false)]
        [Alias('GetState')]
    ...
    try {
        ...
        $normalized = ConvertTo-CachedLogonCountState -Record $observed -Name $name
...
if ($MyInvocation.InvocationName -ne '.') {
    $result = Test-SetCachedLogonCount0
    if ($result.Compliant) {
