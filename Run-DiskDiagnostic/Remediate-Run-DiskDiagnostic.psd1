@{
    ...
    Dependencies = @{
        Modules = @()
        Cmdlets = @('Join-Path', 'Optimize-Volume', 'Set-ItemProperty', 'Start-Process', 'Test-Path', 'Write-Error', 'Write-Output')
    ...
    Test = @{
        Categories = @('Registry', 'File', 'Process', 'Native', 'Destructive')
