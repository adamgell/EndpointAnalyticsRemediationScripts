@{
    ...
    Dependencies = @{
        Modules = @()
        Cmdlets = @('Get-AppxPackage', 'Get-AppxProvisionedPackage', 'Get-FileHash', 'Invoke-WebRequest', 'New-Item', 'Remove-AppxPackage', 'Remove-AppxProvisionedPackage', 'Remove-Item', 'Set-ItemProperty', 'Test-Path', 'Where-Object', 'write-host')
        Executables = @('SetACL.exe')
        Policies = @('HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Communications', 'HKLM\SOFTWARE\Policies\Microsoft\Windows\Windows Chat')
        Endpoints = @('https://github.com/andrew-s-taylor/public/raw/main/De-Bloat/SetACL.exe')
    }
    Configuration = @()
    Risk = @{
        Level = 'High'
    ...
    Test = @{
        Categories = @('Appx', 'Registry', 'Rest', 'File', 'Native', 'Destructive')
